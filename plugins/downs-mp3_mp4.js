...
status: false,
code: 400,
error: "✘ Debes ingresar un enlace para descargarlo."
};
 }
...
status: false,
code: 400,
error: "✘ Debes poner un enlace de YouTube para descargarlo."
};
 }
...
status: true,
code: 200,
result: { title: decrypted.title || "Desconocido", type: format === 'mp3' ? 'audio' : 'video',
 format: format,
...
if (args.length < 1) return conn.sendMessage(m.chat, { text: `𔓕 _Ingrese el comando mas un enlace de *YouTube* para descargarlo._` }, { quoted: m }); //Texto.
let url = args[0];
let mxPlugins = 'downs-mp3_mp4';
let format = command === 'ytmp3' ? 'mp3' : args[1] || '720';
if (!savetube.isUrl(url)) return conn.sendMessage(m.chat, { text: '✘ _Por favor, ingrese un enlace de YouTube para descargarlo.' }, { quoted: m });
try {
conn.sendMessage(m.chat, { text: `_Descargando, espere un momento..._` }, { quoted: m });
...
return conn.sendMessage(m.chat, { text: `*〘 TypeError_Plugin 〙* Invalid command found in directory \`(MDMX-MKTG/home/plugins/${mxPlugins}.js)\`. Please submit a report using the *(#report)* or *(#suggest)* command so we can address the issue.` }, { quoted: m });
 ...
 if (type === 'video') {
await conn.sendMessage(m.chat, { video: { url: download }, caption: txt }, { quoted: m });
 } else {
