...
if (!text) {
return conn.sendMessage(m.chat, { text: `𔒝 _Escriba su nombre o cualquier cosa para crear un logo._\n\n» *Por ejemplo:*\n${usedPrefix + command} Jose` }, { quoted: m }); //Texto.
}
await conn.sendMessage(m.chat, { text: `_Creando logo, espere un momento..._` }, { quoted: m });
...
if (!modelo) {
return conn.sendMessage(m.chat, { text: `⦗ ✘ ⦘ _No se ha encontrado el logo ( *${command}* ), verifica si esta en la lista de logos._` }, { quoted: m });
}

const data = await ephoto(modelo, text);
await conn.sendMessage(m.chat, { image: { url: data }, caption: wm }, { quoted: m });
};
